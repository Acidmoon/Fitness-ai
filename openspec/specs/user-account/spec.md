...
#### Scenario: Inactive account requests profile
- **WHEN** an authenticated inactive user requests `GET /api/user/profile`
- **THEN** the system returns `403 Forbidden`

### Requirement: Active users can update profile fields
The system SHALL allow an authenticated active user to update their username and email through `PUT /api/user/profile`, and newly submitted usernames SHALL not be only digits unless the user is preserving an unchanged legacy numeric username.
...
- **WHEN** an authenticated active user whose current username contains only digits submits the same username unchanged in `PUT /api/user/profile`
- **THEN** the system accepts the update
- **THEN** the username remains unchanged while other valid profile fields can be updated
...
#### Scenario: Inactive account changes password
- **WHEN** an authenticated inactive user submits `PUT /api/user/password`
- **THEN** the system returns `403 Forbidden`

### Requirement: Users can delete their account with password confirmation
The system SHALL allow an authenticated active user to delete their account through `DELETE /api/user/account` when the provided password is correct.

#### Scenario: Successful account deletion
- **WHEN** an authenticated active user submits the correct password to `DELETE /api/user/account`
- **THEN** the system deletes the user account
- **THEN** the system returns a success message

#### Scenario: Wrong password on account deletion
- **WHEN** an authenticated active user submits an incorrect password to `DELETE /api/user/account`
- **THEN** the system returns `400 Bad Request`
...
#### Scenario: Inactive account deletes itself
- **WHEN** an authenticated inactive user submits `DELETE /api/user/account`
- **THEN** the system returns `403 Forbidden`
