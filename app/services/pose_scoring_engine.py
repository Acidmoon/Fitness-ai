"""Pure pose-scoring engine: rule lookup, phase extraction, and quality scoring.

This module is intentionally free of ORM model imports — it operates only on
dicts, rule objects, and value types, so it can be tested offline and reused
by both the sync API path and the async analysis pipeline. Persistence-facing
adapters live in ``app.services.exercise_pose_scoring``.
"""
...
def score_pose_data(exercise: Any, keypoints_data: Any) -> Dict[str, Any]:
    """Score raw keypoint data for an exercise-like object (duck-typed)."""
...
def find_scoring_rule(exercise: Any) -> Optional[ExerciseRule]:
