...
`ready -> down -> bottom -> up -> complete` 这条相位序列是项目规范里的动作周期语义，
不是俯卧撑专属：任何“围绕一个关节角往复”的周期动作（俯卧撑的肘角、深蹲的膝角）
都共享同一台带滞回的阈值状态机，动作差异只体现在阈值与关键点选择上，
因此新增周期动作不应再复制一份状态机。

计次仍委托 `exercise_rules.repetition_counter.count_peak_valley_repetitions`，
本模块只负责相位事件序列与角度统计。
"""
...
class CyclePhaseDetectionResult:
    """一次角度序列的相位事件与计次证据。"""
...
def detect_cycle_phases(
    ...
    *,
    ...
    """按阈值与滞回标记相位事件，并用峰谷计数器给出有效次数。

    `down_angle`/`up_angle` 是动作标准里的“到位角”和“还原角”；
    `hysteresis`/`movement_epsilon` 用于抑制 2D 关键点抖动造成的假切换。
    """
    ...
    return CyclePhaseDetectionResult(
...
def _append_phase(phases: List[Dict[str, Any]], phase: str, sample: AngleLike) -> None:
    phases.append(
        {
            "phase": phase,
